# --- YOLOv12n Video Inference Script with Comprehensive CLI ---
...
class ModelInference:
    def __init__(self, conf_threshold=0.25, iou_threshold=0.45, img_size=640):
        ...
        self.model_paths = {
            'base': 'models/base_yolov12n.pt',  # Default YOLO model
    ...
    def process_image(self, image):
        ...
        # For YOLO models (base and enhanced)
...
guide: Show usage guide and exit''')
    ...
    # Model settings
    parser.add_argument('--model', type=str, choices=['base', 'enhanced'], default='base',
                        help='Model to use for inference in single mode')
    parser.add_argument('--base-model', type=str, default='models/yolo12n.pt',
...
1. Single Model Mode:
   ...
   Example:
   python inference.py --mode single --model base --video test.mp4 --output results/base_output.mp4

2. Comparison Mode:
   Compare the performance of multiple models on the same video.
   
   Example:
   python inference.py --mode compare --video test.mp4 --output-dir results

3. Interactive Mode:
   Run the script with an interactive menu to choose options on the fly.
   
   Example:
   python inference.py --mode interactive --video test.mp4

=== Model Paths ===

You need to specify paths to your model files:

--base-model: Path to original YOLOv12n model (default: models/yolo12n.pt)
...
1. Basic comparison with default settings:
   python inference.py --mode compare --video videos/test_video.mp4

2. Detailed single model processing:
   python inference.py --mode single --model base --video videos/test_video.mp4 --output results/results.mp4 --conf 0.3 --iou 0.5 --save-detections

3. Process specific frames with no display:
   python inference.py --mode single --model enhanced --video videos/long_video.mp4 --start-frame 1000 --max-frames 500 --no-display

4. Compare models with custom thresholds and no plots:
   python inference.py --mode compare --video videos/test_video.mp4 --conf 0.4 --iou 0.6 --no-plots
...
def interactive_mode(inference_engine, video_path, output_dir):
    ...
    while True:
        ...
        print("4. Change detection settings")
        print("5. Change video")
        print("6. Show loaded models")
        print("7. Show guide")
        print("8. Exit")
